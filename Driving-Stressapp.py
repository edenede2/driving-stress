...
def save_as_xlsx_with_highlight_refined(df, scenario, file_name):
    ...
    """
    try:
        # Create a basic XLSX file without formatting
        new_filename = f"test_{file_name.split('.')[0]}.xlsx"
        df.to_excel(new_filename, index=False)

        # Check if file is created successfully
        if not os.path.exists(new_filename):
            raise Exception(f"Failed to create the basic XLSX file: {new_filename}")
        ...
        with pd.ExcelWriter(new_filename, engine='openpyxl', mode='a') as writer:
            ...
            # Define fill patterns, headers, and comments here...
            # (Add your code for cell formatting and adding comments)
...
#         else:
#             print(f"Failed to create the file: {new_filename}")
#             return None  # Return None if file creation failed
